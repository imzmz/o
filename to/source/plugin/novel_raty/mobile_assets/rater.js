...
var rater = document.querySelector("#rater");
rater && Rate({
	element: rater,
	rateCallback: function rateCallback(rating, done) {
		var data = this.element.dataset
		if (!data.tid || !data.fid) {
			return done()
		}
		var tid = parseInt(data.tid);
		var fid = parseInt(data.fid);
		if (Number.isNaN(tid) || Number.isNaN(fid)) {
			return done();
		}
		var url = 'plugin.php?id=novel_raty:raty_process&tid=' + tid + '&fid=' + fid + '&score=' + rating + '&handlekey=raty&mobile=1';
		var self = this
		$.get(url, function (result) {
			if (result.error === 0) {
				self.setRating(result.score);
				popup.open(result.message, "alert")
			} else {
				popup.open(result.message, "alert")
			}
		}, "json");
...
var rater2 = document.querySelector("#rater2");
rater2 && Rate({
	element: rater2,
	rateCallback: function rateCallback(rating, done) {

var rloc = document.getElementById('locate');
var rpage = document.getElementById('readpage');
var rcontent = document.getElementById('content');
var rexternal = document.getElementById('readexternal');
var ctrlEle = document.getElementById('txt_ctrl');
...
var convertword = function(o){
    if(o==null)o = Cookies('simplized');
    if(o==''||o==null) o = 1;
    _simplized = parseInt(o);
    Cookies('simplized', _simplized, 3650000);
    if(_simplized==1){
        StranBody();
        if(rloc){
            rloc.innerHTML = '切换繁体';
            rloc.onclick=function(){
                convertword(2);
            }
        }
    }else{
...
var _simplized = Cookies('simplized');
if(_simplized==''||_simplized==null){
    _simplized = 1;
}

function initLocate(){
    if($('covert-gb')){
        $('covert-gb').innerHTML += '<A href="javascript:void 0;" id="locate"></a>';
    }
    if(_simplized=='2'){
        convertword(2);
    }else{
...
'<li>'+
'皮肤：'+
'<select name="readskin" id="readskin" onchange="previewread()">'+
'<option value="#E8ECEC_#FEFEFE_#222222">默认配色</option>'+
'<option value="#EBE9E9_#fffcfd_#666666">纸质阅读</option>'+
'<option value="#DFF4FF_#E8F2F7_#222222">淡蓝海洋</option>'+
'<option value="#FFF5D6_#FAF6F9_#660000">明黄清俊</option>'+
'<option value="#F7FFEA_#FEFEFE_#265800">绿意淡雅</option>'+
'<option value="#FEDBEF_#FCF4F8_#660066">红粉世家</option>'+
'<option value="#FFF1E1_#FDF9F4_#660000">淡黄明亮</option>'+
'<option value="#52443C_#F5E5D5_#52443C">褐色世界</option>'+
'<option value="#666666_#333333_#DDDDDD">一片漆黑</option>'+
'</select>'+
'</li>'+
'<li>'+
'字体：'+
'<select name="readfont" id="readfont" onchange="previewread()">'+
'<option value="16">默认</option>'+
'<option value="12">小</option>'+
'<option value="18">中</option>'+
'<option value="24">大</option>'+
'<option value="28">超大</option>'+
'<option value="32">巨大</option>'+
'</select>'+
'</li>'+
'<li>'+
'行距：'+
'<select name="readheight" id="readheight" onchange="previewread()">'+
'<option value="200%">默认行距</option>'+
'<option value="150%">紧凑型</option>'+
'<option value="250%">宽松</option>'+
'<option value="300%">巨宽松</option>'+
'</select>'+
'</li>'+
'<li>'+
'文字：'+
'<select name="locateset" id="locateset" onchange="convertword(this.value)">'+
...
'<li class="helperConf">'+
'悬浮工具：'+
'<select name="helper" id="helper" onchange="previewread()">'+
...
function previewread(){
    ...
    if(rcontent){
        //+'||'+$('readshadow').value
        Cookies.set('readset', escape(document.getElementById('readskin').value)+'||'+escape(document.getElementById('readfont').value)+'||'+escape(document.getElementById('readheight').value)+'||'+escape(document.getElementById('helper').value), 86400*360);
        resetread(document.getElementById('readskin').value, document.getElementById('readfont').value, document.getElementById('readheight').value,document.getElementById('helper').value);//, $('readshadow').value
    }
}
...
function toggleConf() {
    if (confStat === 0) {
        confStat = 1;
        ctrlEle.setAttribute('class','')
    } else {
        confStat = 0;
        ctrlEle.setAttribute('class','hide')
    }
}
function resetread(skin, fnt, hgt, tool){
    var colorarr = skin.split(/\_/g);
    document.body.style.background = colorarr[0];
    rpage.style.background = colorarr[1];
    rcontent.style.color = colorarr[2];
    if(rexternal){
        rexternal.style.background = colorarr[1];
    }
    rcontent.style.fontSize = fnt+'px';//$('readfont').value;
    rcontent.style.lineHeight = hgt;//$('readheight').value;
    document.getElementById('tool').style.display=(tool == 1)?'block':'none';
}
function resetconf(skin, fnt, hgt,tool){
   document.getElementById('readskin').value = skin;
   document.getElementById('readfont').value = fnt;
   document.getElementById('readheight').value = hgt;
   document.getElementById('helper').value=tool;
}
var readInit = function() {
    
    ctrlEle.innerHTML = ctrlHtml;
    var _simplized = Cookies('simplized');
    if (_simplized) {
        document.getElementById('locateset').value = _simplized;
        if(_simplized=='2'){
            convertword(2);
        }else{
            convertword(1);
    ...
    if(Cookies.get('readset')){
        var readarr = Cookies.get('readset').split(/\|\|/g);
        if(readarr.length>3){
            resetread(unescape(readarr[0]), unescape(readarr[1]), unescape(readarr[2]),unescape(readarr[3]));
            resetconf(unescape(readarr[0]), unescape(readarr[1]), unescape(readarr[2]),unescape(readarr[3]));
        }
    }
}
document.ready(readInit)
